...
interface SimpMusicUploadDialogProps {
  syncedLyrics: string;
  plainLyrics: string;
  audioFile: File | null;
  onClose: () => void;
...
export default function SimpMusicUploadDialog({
  syncedLyrics,
  plainLyrics,
  audioFile,
  onClose,
}: SimpMusicUploadDialogProps) {
  ...
  const handleUpload = async () => {
    ...
    setStep('uploading');
    setError('');
    setUploadProgress('Uploading lyrics to SimpMusic...');

    try {
      // Prepare the lyrics data for SimpMusic
      const payload = {
        artist: artistName.trim(),
        track: trackName.trim(),
        album: albumName.trim() || 'Unknown Album',
        duration: getDuration(),
        plainLyrics: plainLyrics,
        syncedLyrics: syncedLyrics,
        contributor: contributorName.trim() || 'Lyrica API',
        timestamp: new Date().toISOString(),
      };

      setUploadProgress('Sending lyrics to SimpMusic lyrics database...');

      // Upload to SimpMusic
      const uploadRes = await fetch('/api/simpmusic/submit', {
        method: 'POST',
      ...
      if (!uploadRes.ok) {
        ...
        throw new Error(errorData.message || 'Failed to submit lyrics to SimpMusic');
      }

      const result = await uploadRes.json();
      setUploadProgress('Lyrics submitted successfully to SimpMusic!');
      setStep('success');
    } catch (err) {
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-muted-foreground mb-4">
              Are these lyrics correct? If yes, you can contribute them to SimpMusic to help other music
              enthusiasts enjoy synchronized lyrics. Your contribution will be added to the SimpMusic lyrics database!
            </p>
    ...
    return (
      <Card className="p-6 max-w-lg mx-auto bg-gradient-to-br from-secondary/10 to-secondary/5">
        <div className="text-center">
          ...
          <p className="text-sm text-muted-foreground mb-4">
            Your lyrics have been submitted to SimpMusic. Thank you for contributing to the lyrics library!
          </p>
